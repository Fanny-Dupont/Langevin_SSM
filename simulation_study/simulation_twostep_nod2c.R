####################################################################################     
#                      Code to reproduce the simulations for two-step Langevin     #
...
setwd("/Users/fannydupont/Desktop/PhD/LangevinSSM_Package/simulation")
...
propMissing <- 0 # 0.15 0.20 0.25 0.30 0.40 0.45 0.60 0.70, proportion of missing observations; passed as NA observations to TMB (so corresponding true locations treated as random effects to be estimated)
...
for(isim in 1:nsims){
  ...
  init.mu <- tryCatch(
    {
      init.mu_aniMotum(subDat[[isim]][notNA,], model = "rw", timeSteps = data.frame(
        id = as.character(subDat[[isim]]$id),
        date = as.POSIXlt(subDat[[isim]]$date * 1000)
      ))
    },
    error = function(e) {
      message(sprintf("Error in init.mu_aniMotum for isim = %d: %s", isim, conditionMessage(e)))
      error <<- TRUE
    }
  )
  ...
  subDat[[isim]]$x <- init.mu[,1]
  subDat[[isim]]$y <- init.mu[,2]
  subDat[[isim]]$smaj <- rep(NA,nrow(subDat[[isim]]))
  subDat[[isim]]$smin <- rep(NA,nrow(subDat[[isim]]))
  subDat[[isim]]$eor <- rep(NA,nrow(subDat[[isim]]))
  subDat[[isim]]$x.sd <- rep(NA,nrow(subDat[[isim]]))
  subDat[[isim]]$y.sd <- rep(NA,nrow(subDat[[isim]]))
  

  par <- list(beta=beta,sigma=sigma,gamma=gamma) # do not specify psi because no error
  #init.vel <- subDat[[isim]][,c("vel.x","vel.y")] # true values
  init.vel <- matrix(0,nrow(subDat[[isim]]),2)
  #init.v_mu <- matrix(0,2,nrow(subDat)) 
  for(i in 1:nbAnimals){
    aInd <- which(subDat[[isim]]$id==i)
    init.vel[aInd[-1],1] <- diff(init.mu[aInd,1])/(subDat[[isim]]$dt[aInd[-1]]*exp(-gamma*subDat[[isim]]$dt[aInd[-1]]))
    init.vel[aInd[-1],2] <- diff(init.mu[aInd,2])/(subDat[[isim]]$dt[aInd[-1]]*exp(-gamma*subDat[[isim]]$dt[aInd[-1]]))
  }
  par$vel = init.vel
  ...
  if(useInitialValues) par <- initialValues(subDat[[isim]],model,spatialCovs=spatialCovs[[isim]])
  

  
  # Fit with the fixed parameters initialized at 0
  langFit[[isim]] <- fitLangevin(subDat[[isim]],model=model,spatialCovs=spatialCovs[[isim]],
                                 smoothGradient = ifelse(npoints>0,TRUE,FALSE), npoints = npoints, curweight = curweight, zetaScale = zetaScale,
                                 silent=TRUE, control=list(trace=0),initialInner=initialInner)
...
save(parMat,beta,sigma,gamma,obsPerAnimal,subDat,langFit,langFit_true,prop_truebetter,psi,timeStep,samplingRate,propMissing,measurementError,sca,npoints,curweight,covRange,map,useInitialValues,covTimes,file=paste0("results/two_step_rw_",dataName,"_samplingRate",samplingRate,"_propMissing",propMissing,"_aniMotum",use_aniMotum,"_npoints",npoints,"_curweight",curweight,"_zetaScale",zetaScale,".RData"))
