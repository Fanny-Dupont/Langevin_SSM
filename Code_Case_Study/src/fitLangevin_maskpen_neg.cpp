...
matrix<Type> extract_raster_values(Type x, Type y, 
  ...
  Type res_x = raster_resolution[0];
  Type res_y = raster_resolution[1];

  int n_cols = CppAD::Integer((raster_extent[1] - raster_extent[0]) / res_x);
  int n_rows = CppAD::Integer((raster_extent[3] - raster_extent[2]) / res_y);

  
  Type col_raw = (x - raster_extent[0]) / res_x - Type(0.5);
  Type row_raw = (raster_extent[3] - y) / res_y - Type(0.5);

  int c0 = CppAD::Integer(floor(col_raw));
  int r0 = CppAD::Integer(floor(row_raw));

  matrix<Type> grad_values(2, n_covs);
  grad_values.setZero();

  if(c0 < 0 || c0 >= (n_cols-1) || r0 < 0 || r0 >= (n_rows-1)) {
    return grad_values;
  }
  ...
  for(int i = 0; i < n_covs; i++) {
    int idx = i * (n_rows * n_cols) + r0 * n_cols + c0;

    Type f00 = raster_vals.data()[idx];
    Type f10 = raster_vals.data()[idx + 1];
    Type f01 = raster_vals.data()[idx + n_cols];
    Type f11 = raster_vals.data()[idx + n_cols + 1];

    grad_values(0,i) = ((f10 - f00) * (Type(1.0) - dy) + (f11 - f01) * dy) / res_x;
    grad_values(1,i) = ((f00 - f01) * (Type(1.0) - dx) + (f10 - f11) * dx) / res_y;
  }
  return grad_values;
}
...
{
  Type res_x = mask_resolution[0];
  Type res_y = mask_resolution[1];

  int n_cols = CppAD::Integer((mask_extent[1] - mask_extent[0]) / res_x);
  int n_rows = CppAD::Integer((mask_extent[3] - mask_extent[2]) / res_y);

  Type col_raw = (x - mask_extent[0]) / res_x - Type(0.5);
  Type row_raw = (mask_extent[3] - y) / res_y - Type(0.5);

  int col = CppAD::Integer(floor(col_raw));
  int row = CppAD::Integer(floor(row_raw));
  ...
  Type dx = col_raw - Type(col);
  Type dy = row_raw - Type(row);

  int idx = row * n_cols + col;

  matrix<Type> f(2,2);
  f(0,0) = mask_vals.data()[idx];
  f(1,0) = mask_vals.data()[idx + 1];
  f(0,1) = mask_vals.data()[idx + n_cols];
  f(1,1) = mask_vals.data()[idx + n_cols + 1];

  Type w00 = (Type(1.0) - dx) * (Type(1.0) - dy);
  Type w10 = dx * (Type(1.0) - dy);
  Type w01 = (Type(1.0) - dx) * dy;
  Type w11 = dx * dy;

  value = w00 * f(0,0) + w10 * f(1,0) + w01 * f(0,1) + w11 * f(1,1);
  ...
  for(int id = 0; id < track_starts.size(); id++) {
    ...
    for(int state = 0; state < nbStates; state++) {
      ...
      for(int t = 0; t < (track_length-1); t++) {
        ...
        // nll +=  1e5*pow(mask_val_at_loc(idx),Type(2.0)) + mask_val_at_loc(idx) ;
        // nll +=  1e5*pow(mask_val_at_loc(idx),Type(2.0)) + 0.5*psi + 0.5*sigma.sum();
         nll +=  1e5*pow(mask_val_at_loc(idx),Type(2.0)) + 0.5*tau(0) + 0.5*tau(1) + 0.5*psi + 0.5*pow(((log_sigma.sum() - log(4))/0.3),Type(2.0)); // 0.5*psi = (1/n)p2
      }
    }
